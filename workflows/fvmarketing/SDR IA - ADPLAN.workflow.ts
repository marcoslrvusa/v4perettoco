...
// AiAgent5                           agent                      [AI]
// VectorStoreTool                    toolVectorStore            [AI] [ai_tool] [ai_tool]
// StickyNote71                       stickyNote
// SupabaseVectorStore2               vectorStoreSupabase        [AI] [creds] [ai_vectorStore]
// Supabase5                          supabaseTool               [creds] [ai_tool] [ai_tool]
...
// OpenaiChatModel9                   lmChatOpenAi               [creds] [ai_languageModel] [ai_languageModel]
// Wait3                              wait
// If1                                if
...
// AiAgent5.uses({ ai_languageModel: OpenaiChatModel9, ai_tool: [VectorStoreTool, Supabase5] })
...
// VendedorRemo1.uses({ ai_tool: [VectorStoreTool, Supabase5], ai_languageModel: OpenaiChatModel6 })
// </workflow-map>
...
? $('camposIniciais')?.item?.json?.content?.idMensagem 
  : JSON.parse($json.message?.[0] || '{}').message_id?.toString().trim() 
    ...
    @node({
        id: '7e5e68bf-3b58-4e76-8c00-7d243bc4a278',
        name: 'Segmentos',
...
return [{ json: { telefoneFormatado } }];`,
    ...
    CodeInJavascript3 = {
        jsCode: `const raw = $('If2').item.json.phone;
if (!raw) throw new Error('phone vazio ou nulo');
const digits = raw.toString().replace(/\\D/g, '');
if (!digits) throw new Error('phone sem digitos apos limpeza');
const telefoneFormatado = digits.startsWith('55') ? '+' + digits : '+55' + digits;
return [{ json: { telefoneFormatado } }];`,
    };
